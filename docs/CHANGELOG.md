...
### 生产环境（业务与功能）
1. 【Umi Copilot 智能助手】全面集成企业级 AI 业务助手，支持自然语言工单排产（标准保存态）、全生命周期追溯问答与库存极速穿透速查。
2. 【权限动态治理】AI 知识库与能力插件完全由数据库 `cer_menu` 与 `cer_permission` 动态实时鉴权，严格物理阻断未授权模块提问；AI 抽屉推荐快捷指令根据当前操作员授权动态呈现。
3. 【打印单上查】医疗器械与药品打印任务列表全新上线【上查】下拉功能（选中上查/整单上查），一键穿透反查来源生产工单详情与购货客户源头订单，打通逆向追溯闭环。
4. 【会话与记忆隔离】账号注销或切换登录自动清空静态问答上下文；语义记忆按企业与用户物理分片隔离，杜绝跨账号偏好串扰。
5. 【GMP/GSP合规加固】AI 辅助工单生成严格遵循原生“创建（保存）”状态，由车间主管在列表审核把控流通，实名记录 AI 辅助创建人与审计留痕。
6. 【SOP规范全面重构】全系统标准操作规程增补单据下推四模式、包装码折算弹窗与全部业务表单必填项校验矩阵。
